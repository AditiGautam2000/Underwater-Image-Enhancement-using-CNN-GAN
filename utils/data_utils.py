...
class DataLoader():
    def __init__(self, data_dir, dataset_name, img_res=(256, 256), test_only=False):
        ...
        if not test_only:
            self.trainA_paths = getPaths('./Paired/underwater_imagenet/trainA')  # distorted
            self.trainB_paths = getPaths('./Paired/underwater_imagenet/trainB')  # enhanced
            #self.trainA_paths = getPaths(os.join(self.data_dir, "trainA")) # distorted
            ...
            self.val_paths = getPaths('./Paired/underwater_imagenet/validation') 
